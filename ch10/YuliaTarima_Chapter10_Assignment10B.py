...
class BankAcct(Money):
    def __new__(cls, name, account_number, initial_amount, interest_rate):
    ...
    def calculate_interest(self, days):
        ...
        daily_interest = self._amount * (self.interest_rate / Decimal('100')) / Decimal('365')
...
class TestBankAcct(unittest.TestCase):
    ...
    def test_calculate_interest(self):
        ...
        expected_interest = self.account.balance() * (
                Decimal('2.5') / Decimal('100')) / Decimal('365') * Decimal('30')
        # Compare expected and actual results; Print error message
        # if test fails
